...
import type { Note, NoteRow, CreateNoteInput, UpdateNoteInput, SearchNotesInput } from '../types.js';
...
export class NotesDatabase {
  ...
  search(input: SearchNotesInput): { notes: Note[]; total: number; hasMore: boolean } {
    ...
    const stmt = this.db.prepare(`
      ...
      LIMIT ? OFFSET ?
    `);
    const rows = stmt.all(...values, limit, offset) as NoteRow[];
  ...
  getStats(): { totalNotes: number; projectCount: number; categoryCounts: Record<string, number> } {
    ...
    const projectResult = projectStmt.get() as { count: number };
